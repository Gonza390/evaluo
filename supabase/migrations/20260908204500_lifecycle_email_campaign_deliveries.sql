create table if not exists public.email_campaign_deliveries (
  ...
  constraint email_campaign_deliveries_status_check check (status in ('sending', 'sent')),
  constraint email_campaign_deliveries_campaign_user_key unique (campaign_key, user_id)
);

create index if not exists email_campaign_deliveries_campaign_sent_idx
  on public.email_campaign_deliveries (campaign_key, sent_at desc);
...
grant select, insert, update, delete on table public.email_campaign_deliveries to service_role;
...
as $$
  ...
  calendar_last as (
    select sce.user_id, max(greatest(sce.created_at, sce.event_date::timestamptz)) as last_calendar_at
