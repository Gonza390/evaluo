drop function if exists public.get_reactivation_next_subject_candidates(timestamptz, integer);

create or replace function public.get_reactivation_next_subject_candidates(
  p_cutoff timestamptz,
  p_min_cutoff timestamptz default null,
...
as $$
  ...
  where u.email is not null
    ...
    and (p_min_cutoff is null or a.last_active_at > p_min_cutoff)
    and not exists (
      select 1
...
revoke all on function public.get_reactivation_next_subject_candidates(timestamptz, timestamptz, integer)
from public, anon, authenticated;
grant execute on function public.get_reactivation_next_subject_candidates(timestamptz, timestamptz, integer)
to service_role;
