...
async function handle(request: Request) {
  ...
  try {
    const result = await runReactivationNextSubjectDispatch({ dryRun });
    return NextResponse.json(result);
  } catch (error) {
    logError('reactivationNextSubject.route', error);
    return NextResponse.json(
      { success: false, message: 'No pudimos procesar la campaña de reactivación.' },
      { status: 500 }
    );
  }
}
