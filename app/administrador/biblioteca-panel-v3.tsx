...
import {
  crearMateriaEnCarreraAdministrador,
  vincularMateriaExistenteAdministrador,
} from './materia-hierarchy-actions';
...
function Row({ title, subtitle, icon, onClick }: { title: string; subtitle: string; icon: ReactNode; onClick: () => void }) {
  return (
...
function Empty({ title, description }: { title: string; description: string }) {
  return (
    <div className="rounded-2xl border border-dashed border-[#d7deeb] bg-white px-5 py-9 text-center">
...
function TextForm({ title, placeholder, value, pending, onChange, onSubmit, onCancel }: { title: string; placeholder: string; value: string; pending: boolean; onChange: (v: string) => void; onSubmit: () => void; onCancel: () => void }) {
  return (
    <div className="mb-5 rounded-2xl border border-[#dce3f0] bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <input autoFocus value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') onSubmit(); if (e.key === 'Escape') onCancel(); }} className="h-10 flex-1 rounded-xl border border-[#dce3f0] bg-white px-3 text-[14px] text-[#1d2a44] outline-none focus:border-[#315efb] focus:ring-2 focus:ring-[#315efb]/10" />
...
function LinkForm({ options, value, pending, onChange, onSubmit, onCancel }: { options: BibliotecaMateriaOption[]; value: string; pending: boolean; onChange: (v: string) => void; onSubmit: () => void; onCancel: () => void }) {
  return (
    <div className="mb-5 rounded-2xl border border-[#dce3f0] bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
      ...
      {options.length ? <div className="flex flex-col gap-2 sm:flex-row"><select value={value} onChange={(e) => onChange(e.target.value)} className="h-10 flex-1 rounded-xl border border-[#dce3f0] bg-white px-3 text-[14px] text-[#1d2a44]"><option value="">Seleccionar materia</option>{options.map((m) => <option key={m.id} value={m.id}>{m.nombre}</option>)}</select><button type="button" disabled={pending || !value} onClick={onSubmit} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-[#315efb] px-4 text-[13px] font-semibold text-white disabled:opacity-50">{pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Link2 className="h-4 w-4" />}Vincular</button></div> : <p className="rounded-xl bg-[#f7f9fc] px-3 py-3 text-[12px] text-[#7d899f]">No hay otras materias disponibles para vincular.</p>}
    </div>
  );
}
...
export function BibliotecaPanel({ universidades, materias }: { overview: BibliotecaOverviewStats; universidades: BibliotecaUniversidadOption[]; carreras: BibliotecaCarreraOption[]; materias: BibliotecaMateriaOption[]; carrerasSimuladores: BibliotecaCarreraSimuladorRow[] }) {
  ...
  const universidad = universidades.find((x) => x.id === universidadId) ?? null;
  const facultad = facultades.find((x) => x.id === facultadId) ?? null;
  const carrera = carrerasUniversidad.find((x) => x.id === carreraId) ?? null;
  const level = carreraId ? 'materias' : facultadId ? 'facultad' : universidadId ? 'universidad' : 'universidades';
  const carrerasDirectas = useMemo(() => carrerasUniversidad.filter((x) => !x.facultadId), [carrerasUniversidad]);
  const carrerasFacultad = useMemo(() => carrerasUniversidad.filter((x) => x.facultadId === facultadId), [carrerasUniversidad, facultadId]);
  const materiasVisibles = useMemo(() => carreraId ? materias.filter((m) => m.carreraIds.includes(carreraId)) : [], [carreraId, materias]);
  const materiasVinculables = useMemo(() => {
    if (!carreraId) return [];
    const careerIds = new Set(carrerasUniversidad.map((x) => x.id));
    const actuales = new Set(materiasVisibles.map((m) => m.id));
    return materias.filter((m) => !actuales.has(m.id) && m.carreraIds.some((id) => careerIds.has(id))).sort((a, b) => a.nombre.localeCompare(b.nombre, 'es'));
  ...
  const loadUniversity = async (id: string) => { setLoading(true); const result = await obtenerEstructuraUniversidadAdministrador(id); setLoading(false); if (!result.success) { toast({ description: result.message ?? 'No pudimos cargar la universidad.', variant: 'destructive' }); return false; } setFacultades(result.facultades ?? []); setCarrerasUniversidad(result.carreras ?? []); return true; };
  ...
  const submitCreate = () => {
    const nombre = newName.trim(); if (!nombre || !createMode) return;
    startTransition(async () => {
      let result: { success: boolean; message: string } | null = null;
      if (createMode === 'universidad') result = await crearUniversidadBibliotecaAdministrador(nombre);
      else if (createMode === 'facultad' && universidadId) result = await crearFacultadAdministrador({ nombre, universidadId });
      else if (createMode === 'carrera-directa' && universidadId) result = await crearCarreraDirectaUniversidadAdministrador({ nombre, universidadId });
      else if (createMode === 'carrera-facultad' && universidadId && facultadId) result = await crearCarreraEnFacultadAdministrador({ nombre, universidadId, facultadId });
      else if (createMode === 'materia' && carreraId) result = await crearMateriaEnCarreraAdministrador({ nombre, carreraId });
      if (!result) return; toast({ description: result.message, variant: result.success ? 'default' : 'destructive' }); if (!result.success) return;
      const mode = createMode; closeInputs(); if (universidadId && mode !== 'materia') await loadUniversity(universidadId); router.refresh();
    });
  };
  const submitLink = () => { if (!materiaToLink || !carreraId) return; startTransition(async () => { const result = await vincularMateriaExistenteAdministrador({ materiaId: materiaToLink, carreraId }); toast({ description: result.message, variant: result.success ? 'default' : 'destructive' }); if (result.success) { closeInputs(); router.refresh(); } }); };

  const createCopy = createMode === 'universidad' ? { title: 'Nueva universidad', placeholder: 'Nombre de la universidad' } : createMode === 'facultad' ? { title: 'Nueva facultad', placeholder: 'Nombre de la facultad' } : createMode === 'carrera-directa' ? { title: 'Nueva carrera', placeholder: 'Nombre de la carrera' } : createMode === 'carrera-facultad' ? { title: `Nueva carrera en ${facultad?.nombre ?? 'la facultad'}`, placeholder: 'Nombre de la carrera' } : { title: `Nueva materia en ${carrera?.nombre ?? 'la carrera'}`, placeholder: 'Nombre de la materia' };
  const back = level === 'universidad' ? goRoot : level === 'facultad' ? goUniversity : facultadId ? goFaculty : goUniversity;
  ...
  return (
    <section className="mx-auto max-w-5xl">
      ...
      {level !== 'universidades' && <button type="button" onClick={back} className="mb-4 inline-flex items-center gap-1.5 text-[12px] font-medium text-[#6f7c96]"><ArrowLeft className="h-3.5 w-3.5" />Volver</button>}
      ...
      {loading ? <div className="flex min-h-[220px] items-center justify-center rounded-2xl border border-[#e4e9f2] bg-white"><span className="flex items-center gap-2 text-[13px] text-[#7f8aa3]"><Loader2 className="h-4 w-4 animate-spin" />Cargando estructura...</span></div>
      : level === 'universidades' ? (universidades.length ? <div className="grid gap-2.5">{universidades.map((u) => <Row key={u.id} title={u.nombre} subtitle="Abrir universidad" icon={<School className="h-5 w-5" />} onClick={() => void openUniversity(u.id)} />)}</div> : <Empty title="No hay universidades" description="Añadí la primera universidad para comenzar." />)
      : level === 'universidad' ? <div className="space-y-8">
          {facultades.length > 0 && <div><Section title="Facultades" description="Entrá a una facultad para ver sus carreras." /><div className="grid gap-2.5">{facultades.map((f) => <Row key={f.id} title={f.nombre} subtitle="Ver carreras" icon={<Building2 className="h-5 w-5" />} onClick={() => openFaculty(f.id)} />)}</div></div>}
          {carrerasDirectas.length > 0 && <div><Section title="Carreras directas" description="Carreras que dependen directamente de la universidad." /><div className="grid gap-2.5">{carrerasDirectas.map((c) => <Row key={c.id} title={c.nombre} subtitle="Ver materias" icon={<GraduationCap className="h-5 w-5" />} onClick={() => openCareer(c.id)} />)}</div></div>}
          {facultades.length === 0 && carrerasDirectas.length === 0 && <Empty title="Todavía no hay estructura cargada" description="Podés añadir una facultad o una carrera directa." />}
        </div>
