  ...
  const createCurrentLevel = () => {
    ...
    startTransition(async () => {
      ...
      if (level === 'universidades') {
        ...
        result = await crearCarreraEnFacultadAdministrador({
          nombre,
          universidadId,
          facultadId,
        });
    ...
    <section className="mx-auto max-w-5xl">
      <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-[1.55rem] font-semibold tracking-[-0.045em] text-[#1d2a44]">
            Estructura académica
          </h2>
          <p className="mt-1 text-[13px] text-[#7f8aa3]">
            Navegá de universidad a facultad, carrera y materia sin mezclar niveles.
          </p>
        </div>
      ...
      <div className="mb-5 flex min-h-8 flex-wrap items-center gap-1.5 text-[12px]">
        <button type="button" onClick={goToRoot} className="font-medium text-[#315efb] hover:underline">
          Universidades
        </button>
        {universidadActiva ? (
          <>
            <ChevronRight className="h-3.5 w-3.5 text-[#a5afc1]" />
            <button type="button" onClick={goToUniversity} className="font-medium text-[#315efb] hover:underline">
              {universidadActiva.nombre}
            </button>
          </>
        ) : null}
        {facultadId ? (
      ...
      {loadingChildren ? (
        <div className="flex min-h-[220px] items-center justify-center rounded-2xl border border-[#e4e9f2] bg-white">
          <div className="flex items-center gap-2 text-[13px] text-[#7f8aa3]">
          ...
          <div className="grid gap-2.5">
            {universidades.map((universidad) => (
              <NavigationRow
                ...
                subtitle="Ver facultades"
                icon={<School className="h-5 w-5" />}
                onClick={() => void openUniversity(universidad.id)}
              />
            ))}
          </div>
        ) : (
          ...
          <div className="grid gap-2.5">
            {facultades.map((facultad) => (
              <NavigationRow
                ...
                subtitle="Ver carreras"
                icon={<Building2 className="h-5 w-5" />}
                onClick={() => openFaculty(facultad.id)}
              />
            ))}
            {hasLegacyCareers ? (
              <NavigationRow
                title="Carreras sin facultad asignada"
                subtitle="Datos existentes anteriores a la nueva estructura"
                icon={<Building2 className="h-5 w-5" />}
                onClick={() => openFaculty(LEGACY_FACULTY_ID)}
              />
            ) : null}
          </div>
        ) : (
          <EmptyState
            title="Todavía no hay facultades"
            description={`Añadí la primera facultad dentro de ${universidadActiva?.nombre ?? 'esta universidad'}.`}
          />
        )
      ) : level === 'carreras' ? (
        carrerasVisibles.length > 0 ? (
          ...
          <EmptyState
            ...
            description={
              facultadId === LEGACY_FACULTY_ID
                ? 'No hay carreras pendientes de asignar a una facultad.'
                : `Añadí la primera carrera dentro de ${facultadActiva?.nombre ?? 'esta facultad'}.`
            }
          />
        )
        ...
        <div className="grid gap-2.5">
          {materiasVisibles.map((materia) => (
            <div
              key={materia.id}
              className="flex items-center gap-3 rounded-2xl border border-[#e4e9f2] bg-white px-4 py-4 shadow-[0_5px_18px_rgba(15,23,42,0.035)]"
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#eef3ff] text-[#315efb]">
        ...
        <EmptyState
          ...
          description={`Añadí la primera materia dentro de ${carreraActiva?.nombre ?? 'esta carrera'}.`}
        />
      )}
    </section>
  );
