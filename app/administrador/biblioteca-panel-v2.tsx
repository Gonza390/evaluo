...
function SectionTitle({ title, description }: { title: string; description: string }) {
  return (
    <div className="mb-3">
      <h3 className="text-[14px] font-semibold text-[#25324b]">{title}</h3>
...
function LinkMateriaForm({
  ...
  options: BibliotecaMateriaOption[];
  value: string;
  pending: boolean;
  onChange: (value: string) => void;
  onSubmit: () => void;
  ...
  return (
    <div className="mb-5 rounded-2xl border border-[#dce3f0] bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          <p className="text-[13px] font-semibold text-[#1d2a44]">Vincular materia existente</p>
          <p className="mt-1 text-[12px] text-[#8a95ab]">Solo aparecen materias que ya pertenecen a esta universidad.</p>
        </div>
        <button type="button" onClick={onCancel} className="text-[#8994a9] hover:text-[#315efb]" aria-label="Cancelar">
          <X className="h-4 w-4" />
        </button>
      </div>
      {options.length ? (
        <div className="flex flex-col gap-2 sm:flex-row">
          <select
            value={value}
            onChange={(event) => onChange(event.target.value)}
            className="h-10 flex-1 rounded-xl border border-[#dce3f0] bg-white px-3 text-[14px] text-[#1d2a44] outline-none focus:border-[#315efb] focus:ring-2 focus:ring-[#315efb]/10"
          >
        ...
        <p className="rounded-xl bg-[#f7f9fc] px-3 py-3 text-[12px] text-[#7d899f]">
          No hay otras materias de esta universidad disponibles para vincular.
        </p>
      )}
    </div>
  );
...
export function BibliotecaPanel({
  ...
  const [linkingMateria, setLinkingMateria] = useState(false);
  ...
  const carrerasDirectas = useMemo(
    () => carrerasUniversidad.filter((item) => !item.facultadId),
    [carrerasUniversidad]
  );

  const carrerasFacultad = useMemo(
    () => carrerasUniversidad.filter((item) => item.facultadId === facultadId),
  ...
  const materiasVinculables = useMemo(() => {
    if (!carreraId || !universidadId) return [];
    const careerIds = new Set(carrerasUniversidad.map((item) => item.id));
    const currentIds = new Set(materiasVisibles.map((item) => item.id));
    return materias
      .filter((item) => !currentIds.has(item.id) && item.carreraIds.some((id) => careerIds.has(id)))
      .sort((a, b) => a.nombre.localeCompare(b.nombre, 'es'));
  }, [carreraId, universidadId, carrerasUniversidad, materias, materiasVisibles]);

  const closeInputs = () => {
    setCreateMode(null);
    setNewName('');
    setLinkingMateria(false);
    setMateriaToLink('');
  };
  ...
  const loadUniversity = async (id: string) => {
    ...
    setCarrerasUniversidad(result.carreras ?? []);
    return true;
  };
  ...
  const submitCreate = () => {
    ...
    startTransition(async () => {
      ...
      if (createMode === 'universidad') {
        ...
        result = await crearMateriaEnCarreraAdministrador({ nombre, carreraId });
      }
    ...
    <section className="mx-auto max-w-5xl">
      <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[13px] text-[#7f8aa3]">Universidad → carrera o facultad → carrera → materias.</p>
        </div>

        <div className="flex flex-wrap gap-2">
          {level === 'universidades' ? (
            <button type="button" onClick={() => beginCreate('universidad')} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#315efb] px-4 text-[13px] font-semibold text-white hover:bg-[#254ee0]">
              <Plus className="h-4 w-4" /> Añadir universidad
            </button>
          ) : null}
          {level === 'universidad' ? (
            <>
              <button type="button" onClick={() => beginCreate('carrera-directa')} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#315efb] px-4 text-[13px] font-semibold text-white hover:bg-[#254ee0]">
                <Plus className="h-4 w-4" /> Añadir carrera
              </button>
              <button type="button" onClick={() => beginCreate('facultad')} className="inline-flex h-10 items-center gap-2 rounded-xl border border-[#cfd8e9] bg-white px-4 text-[13px] font-semibold text-[#315efb] hover:bg-[#f6f8ff]">
                <Plus className="h-4 w-4" /> Añadir facultad
              </button>
            </>
          ) : null}
          {level === 'facultad' ? (
            <button type="button" onClick={() => beginCreate('carrera-facultad')} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#315efb] px-4 text-[13px] font-semibold text-white hover:bg-[#254ee0]">
              <Plus className="h-4 w-4" /> Añadir carrera
            </button>
          ) : null}
          {level === 'materias' ? (
            <>
              <button type="button" onClick={() => beginCreate('materia')} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#315efb] px-4 text-[13px] font-semibold text-white hover:bg-[#254ee0]">
                <Plus className="h-4 w-4" /> Crear materia
              </button>
              <button
                type="button"
                onClick={() => {
                  ...
                  setNewName('');
                  setMateriaToLink('');
                  setLinkingMateria(true);
                }}
                className="inline-flex h-10 items-center gap-2 rounded-xl border border-[#cfd8e9] bg-white px-4 text-[13px] font-semibold text-[#315efb] hover:bg-[#f6f8ff]"
              >
      ...
      <div className="mb-4 flex min-h-7 flex-wrap items-center gap-1.5 text-[12px]">
        ...
        {universidad ? <><ChevronRight className="h-3.5 w-3.5 text-[#a5afc1]" /><button type="button" onClick={goUniversity} className="font-medium text-[#315efb] hover:underline">{universidad.nombre}</button></> : null}
        {facultad ? <><ChevronRight className="h-3.5 w-3.5 text-[#a5afc1]" /><button type="button" onClick={goFaculty} className="font-medium text-[#315efb] hover:underline">{facultad.nombre}</button></> : null}
        {carrera ? <><ChevronRight className="h-3.5 w-3.5 text-[#a5afc1]" /><span className="font-medium text-[#65718a]">{carrera.nombre}</span></> : null}
      </div>
      ...
      {createMode ? <CreateForm title={createCopy.title} placeholder={createCopy.placeholder} value={newName} pending={pending} onChange={setNewName} onSubmit={submitCreate} onCancel={closeInputs} /> : null}
      {linkingMateria ? <LinkMateriaForm options={materiasVinculables} value={materiaToLink} pending={pending} onChange={setMateriaToLink} onSubmit={submitLink} onCancel={closeInputs} /> : null}
      ...
      {loading ? (
        ...
        universidades.length ? <div className="grid gap-2.5">{universidades.map((item) => <NavigationRow key={item.id} title={item.nombre} subtitle="Ver carreras y facultades" icon={<School className="h-5 w-5" />} onClick={() => void openUniversity(item.id)} />)}</div> : <EmptyState title="No hay universidades" description="Añadí la primera universidad para comenzar." />
      ) : level === 'universidad' ? (
        <div className="space-y-8">
          <div>
            <SectionTitle title="Carreras" description="Carreras vinculadas directamente a la universidad." />
            {carrerasDirectas.length ? <div className="grid gap-2.5">{carrerasDirectas.map((item) => <NavigationRow key={item.id} title={item.nombre} subtitle="Ver materias" icon={<GraduationCap className="h-5 w-5" />} onClick={() => openCareer(item.id)} />)}</div> : <EmptyState title="No hay carreras directas" description="Podés añadir una carrera sin crear una facultad." />}
          </div>
          <div>
            <SectionTitle title="Facultades" description="Opcional. Usalas cuando la universidad se organiza por facultades." />
            {facultades.length ? <div className="grid gap-2.5">{facultades.map((item) => <NavigationRow key={item.id} title={item.nombre} subtitle="Ver carreras" icon={<Building2 className="h-5 w-5" />} onClick={() => openFaculty(item.id)} />)}</div> : <EmptyState title="No hay facultades" description="No es obligatorio crear una facultad para añadir carreras." />}
          </div>
        </div>
      ) : level === 'facultad' ? (
        carrerasFacultad.length ? <div className="grid gap-2.5">{carrerasFacultad.map((item) => <NavigationRow key={item.id} title={item.nombre} subtitle="Ver materias" icon={<GraduationCap className="h-5 w-5" />} onClick={() => openCareer(item.id)} />)}</div> : <EmptyState title="Todavía no hay carreras" description={`Añadí la primera carrera dentro de ${facultad?.nombre ?? 'esta facultad'}.`} />
      ...
      ) : <EmptyState title="Todavía no hay materias" description={`Creá la primera materia dentro de ${carrera?.nombre ?? 'esta carrera'} o vinculá una existente.`} />}
    </section>
  );
}
