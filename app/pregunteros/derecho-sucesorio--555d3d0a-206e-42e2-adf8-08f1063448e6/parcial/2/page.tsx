...
import { PregunteroDerechoSucesorioPlanExperiment } from '@/components/marketing/preguntero-derecho-sucesorio-plan-experiment';
...
const PAGE_PATH = `/pregunteros/${MATERIA_SLUG}/parcial/2`;
...
export default async function DerechoSucesorioParcial2Page({ searchParams }: PageProps) {
  ...
  const resumenHref = `/resumenes/${MATERIA_SLUG}`;
  ...
  const breadcrumbData = buildBreadcrumbJsonLd([
    ...
    { name: title, path: canonicalHref || PAGE_PATH },
  ]);
  ...
  return (
    <div className="min-h-screen w-full overflow-x-clip bg-white">
      ...
      <main>
        <PregunteroDerechoSucesorioPlanExperiment
          title={title}
          materiaNombre={data.materiaNombre}
          universidadNombre={data.universidadNombre}
          carreraNombre={data.carreraNombre}
