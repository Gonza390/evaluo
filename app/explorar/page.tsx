...
function getUniversityBadge(universidad: Universidad, topCarreras: number, topMaterias: number) {
...
function getActivityLabel(universidad: Universidad) {
...
function ExplorarContent() {
  ...
  useEffect(() => {
    ...
    async function fetchUniversidades() {
      ...
      try {
        ...
        const enrichedUniversidades = (universidadesResult.data ?? []).map((universidad) => ({
          id: universidad.id,
  ...
  return (
    <div className="animate-page-enter mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
      <section className="surface-panel animate-surface-reveal overflow-hidden px-4 py-5 sm:px-6 sm:py-7">
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 text-[2rem] font-black tracking-[-0.05em] text-slate-950 sm:text-[2.6rem]">
          ...
          <div className="grid grid-cols-3 gap-3 sm:min-w-[360px]">
            <div className="surface-card rounded-[var(--radius-card)] bg-slate-50/90 px-3 py-3 shadow-none">
              <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-slate-400">Universidades</p>
              <p className="mt-2 text-xl font-black text-slate-950">{universidades.length}</p>
            </div>
            <div className="surface-card rounded-[var(--radius-card)] bg-slate-50/90 px-3 py-3 shadow-none">
              <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-slate-400">Carreras</p>
                  ...
                  <CardContent className="space-y-4 pt-0">
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-6 text-slate-500">
                        {activityLabel}. Entra para explorar sus carreras, materias y materiales publicados.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="rounded-2xl border border-slate-100 bg-slate-50/90 px-3 py-3">
                        <div className="flex items-center gap-2 text-slate-500">
                          ...
                          <span className="text-[11px] font-bold uppercase tracking-[0.16em]">Carreras</span>
                        </div>
                        <p className="mt-2 text-xl font-black text-slate-950">{universidad.carrerasCount}</p>
                      </div>
                      <div className="rounded-2xl border border-slate-100 bg-slate-50/90 px-3 py-3">
                        <div className="flex items-center gap-2 text-slate-500">
                          <BookOpen className="h-4 w-4" />
                          <span className="text-[11px] font-bold uppercase tracking-[0.16em]">Materias</span>
                        </div>
                        <p className="mt-2 text-xl font-black text-slate-950">{universidad.materiasCount}</p>
                      </div>
                    </div>
