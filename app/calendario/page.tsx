...
const CALENDAR_TOUR_STEPS: CalendarTourStep[] = [
  ...
  },
  {
    ...
    title: 'Elegí un día',
    description:
      'Tocá cualquier día del calendario para ver su contenido o cargar una nueva fecha en ese momento.',
  },
  {
    id: 'composer',
    ...
    async function loadEvents() {
      ...
      setEvents(
        (data ?? []).map((event) => ({
          ...
          examInstance:
            event.exam_instance === '1' ||
            event.exam_instance === '2' ||
            event.exam_instance === 'integrador'
              ? event.exam_instance
              : null,
          sourcePayload:
  ...
  const handleCreateEvent = async () => {
    ...
    setEvents((currentEvents) => [
      ...currentEvents,
      {
        ...
        examInstance:
          data.exam_instance === '1' ||
          data.exam_instance === '2' ||
          data.exam_instance === 'integrador'
            ? data.exam_instance
            : null,
        sourcePayload:
      ...
      <div className="mx-auto w-full max-w-[1080px] min-w-0 space-y-4 overflow-x-clip sm:space-y-6">
        <section className="overflow-hidden rounded-[32px] border border-slate-200/80 bg-[linear-gradient(135deg,#ffffff_0%,#f7faff_50%,#edf4ff_100%)] shadow-[0_30px_90px_rgba(15,23,42,0.10)]">
          <div className="relative px-1.5 py-1.5 min-[380px]:px-2 min-[380px]:py-2 sm:px-4 sm:py-4">
            ...
            <div className="min-w-0 overflow-hidden rounded-[22px] border border-slate-200/80 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:rounded-[28px]">
              <div
                ref={monthTourRef}
                className={`flex flex-col gap-2 border-b border-slate-200/80 bg-[linear-gradient(180deg,#ffffff_0%,#f8fbff_100%)] px-4 py-3 sm:px-5 lg:flex-row lg:items-center lg:justify-between ${
                  showCalendarTour && currentCalendarTourStep.id === 'month'
                    ? 'relative z-[70] shadow-[0_24px_70px_rgba(15,23,42,0.12)] ring-1 ring-[#BFD4FF]'
                    : ''
                }`}
              >
                <div className="relative flex min-w-0 items-center justify-between gap-1 min-[380px]:justify-start min-[380px]:gap-2">
                  <button
                    type="button"
                    onClick={() => setVisibleMonth((current) => addMonths(current, -1))}
                    className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-transparent text-slate-500 transition hover:border-slate-200 hover:bg-white hover:text-slate-900"
                  ...
                  {showCalendarTour && currentCalendarTourStep.id === 'month' ? (
                    <TourCard
                      ...
                      className="pointer-events-auto absolute top-full left-0 z-[80] mt-4 max-sm:fixed max-sm:inset-x-4 max-sm:top-auto max-sm:bottom-[5.75rem] max-sm:translate-x-0 max-sm:translate-y-0"
                    />
                  ) : null}
                </div>

                <div className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-2 min-[420px]:flex min-[420px]:flex-wrap">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => {
                    ...
                    className="h-8 rounded-xl border border-slate-200 bg-white px-3 text-slate-700 shadow-sm hover:bg-white hover:text-slate-900"
                  >
                    Hoy
                  </Button>
                  <Button
                    ...
                    className="h-8 min-w-0 rounded-xl bg-gradient-to-r from-[#2563EB] to-[#6366F1] px-3 text-white shadow-[0_12px_28px_rgba(37,99,235,0.24)] hover:opacity-95 sm:px-4"
                  >
              ...
              >
                {WEEKDAY_LABELS.map((label) => (
                  <div
                    key={label}
                    className="min-w-0 border-r border-slate-200/80 px-0.5 py-2 text-center text-[11px] font-bold text-slate-600 last:border-r-0 sm:px-3 sm:text-[12px]"
                  >
              ...
              >
                {showCalendarTour && currentCalendarTourStep.id === 'grid' ? (
                  <TourCard
                    ...
                    onPrevious={handleCalendarTourPrevious}
                    onClose={closeCalendarTour}
                    className="pointer-events-auto absolute top-4 left-1/2 z-[80] -translate-x-1/2 max-sm:fixed max-sm:inset-x-4 max-sm:top-auto max-sm:bottom-[5.75rem] max-sm:translate-x-0 max-sm:translate-y-0"
                  />
                ) : null}
                {monthGrid.map((date) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group flex min-h-[58px] min-w-0 flex-col gap-1 border-r border-b px-1 py-1.5 text-left transition last:border-r-0 hover:bg-white min-[380px]:min-h-[64px] min-[380px]:px-1.5 sm:min-h-[74px] sm:gap-1.5 sm:px-2.5 sm:py-2 lg:min-h-[86px] ${
                        ...
                        <span
                          ...
                          style={{
                            backgroundColor: isToday
                              ? '#2563EB'
                              : isSelected
                                ? 'color-mix(in oklch, #dce8ff 55%, var(--background))'
                                : 'transparent',
                            color: isToday
                              ? '#ffffff'
                              : isSelected
                                ? 'color-mix(in oklch, #2563EB 75%, var(--foreground))'
                                : undefined,
                          }}
                        >
                          {date.getDate()}
                        ...
                        {cellEvents.slice(0, 2).map((event) => (
                          <div
                            ...
                            style={{
                              backgroundColor:
                                event.type === 'exam'
                                  ? 'color-mix(in oklch, #fff4df 62%, var(--background))'
                                  : 'color-mix(in oklch, #edf4ff 62%, var(--background))',
                              borderColor:
                                event.type === 'exam'
                                  ? 'color-mix(in oklch, #f6ddb0 52%, var(--border))'
                                  : 'color-mix(in oklch, #d5e4ff 52%, var(--border))',
                              color:
                                event.type === 'exam'
                                  ? 'color-mix(in oklch, #b7791f 72%, var(--foreground))'
                                  : 'color-mix(in oklch, #2563EB 72%, var(--foreground))',
                            }}
                          >
                            {event.title}
                          </div>
        ...
        <DialogContent className="h-[min(78vh,640px)] w-[min(calc(100vw-1rem),360px)] max-w-[360px] overflow-hidden rounded-[22px] border border-slate-200 bg-white p-0 text-slate-900 shadow-[0_24px_70px_rgba(15,23,42,0.14)] md:h-auto md:max-h-[76vh] md:w-[680px] md:max-w-[680px] lg:w-[760px] lg:max-w-[760px]">
          <div className="relative h-full overflow-y-auto px-3 py-3 sm:px-4 sm:py-4 md:max-h-[76vh]">
            {showCalendarTour && currentCalendarTourStep.id === 'composer' ? (
              <div className="pointer-events-none absolute inset-0 z-[70] bg-white/18 backdrop-blur-[3px]" />
            ) : null}
            <div className="grid gap-3 lg:grid-cols-[minmax(0,1.45fr)_240px] lg:items-start">
              ...
              <div className="rounded-[18px] border border-slate-200 bg-white p-1">
            ...
            <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1.45fr)_240px] lg:items-start">
              <div ref={composerTourRef} className="relative">
                <div
                  className={`h-[332px] overflow-hidden ${
                    showCalendarTour && currentCalendarTourStep.id === 'composer'
                      ? 'relative z-[80] rounded-[24px] shadow-[0_24px_70px_rgba(15,23,42,0.12)] ring-1 ring-[#BFD4FF]'
                      : ''
                  }`}
                >
                  {formState.type === 'exam' ? (
                    <div className="grid h-full grid-rows-[auto_auto_minmax(112px,1fr)] gap-3">
                      ...
                      <div className="flex min-h-[112px] flex-col space-y-2">
                        ...
                        {careerMateriaLoading ? (
                          <div className="flex min-h-[112px] items-center rounded-[16px] border border-dashed border-slate-200 bg-white px-3 py-3 text-[13px] text-slate-500">
                          ...
                          <div className="max-h-[112px] min-h-[112px] overflow-y-auto rounded-[16px] border border-slate-200 bg-white p-1 shadow-sm">
                            {filteredCareerMateriaOptions.map((materia) => (
                              <button
                                ...
                                className="flex w-full items-center rounded-xl px-3 py-2 text-left text-[13px] text-slate-700 transition hover:bg-white hover:text-slate-950"
                              >
                                {materia.nombre}
                              </button>
                            ))}
                          ...
                          <div className="flex min-h-[112px] items-center rounded-[16px] border border-dashed border-slate-200 bg-white px-3 py-3 text-[13px] text-slate-500">
                            Cuando tu carrera tenga materias asociadas, aparecerán aquí para
                            elegirlas.
                          </div>
                        ) : null}
                      </div>
                    ...
                    <div className="grid h-full grid-rows-[auto_auto_minmax(112px,1fr)] gap-3">
                      ...
                      <div className="flex min-h-[112px] flex-col justify-between rounded-[16px] border border-dashed border-slate-200 bg-white px-3 py-3 text-[13px] text-slate-500">
                        <p className="text-[12px] font-semibold tracking-[0.14em] text-slate-500 uppercase">
                          Organizacion
                        </p>
                        <p>
                          Guardá la entrega con un nombre claro para ubicarla rápido en
                          recordatorios, calendario y seguimiento semanal.
                        </p>
                        <p className="text-[12px] text-slate-500">
                          Ejemplo: TP final de Derecho Privado.
                        </p>
                      </div>
                      <div className="hidden rounded-[16px] border border-dashed border-slate-200 bg-white px-3 py-3 text-[13px] text-slate-500">
                        Guardá la entrega con un nombre claro para poder usar esta información
                        después en recordatorios, analíticas o planificación.
                      </div>
                    </div>
                  )}
                </div>
                ...
                {showCalendarTour && currentCalendarTourStep.id === 'composer' ? (
                  <TourCard
                    ...
                    onPrevious={handleCalendarTourPrevious}
                    onClose={closeCalendarTour}
                    className="pointer-events-auto absolute top-full left-1/2 z-[90] mt-3 -translate-x-1/2 max-sm:fixed max-sm:inset-x-4 max-sm:top-auto max-sm:bottom-[5.75rem] max-sm:translate-x-0 max-sm:translate-y-0"
                  />
                ) : null}
              </div>
              ...
              <div className="space-y-3">
                ...
                {selectedDateEvents.length > 0 ? (
                  <div className="max-h-40 space-y-2 overflow-y-auto rounded-[16px] border border-slate-200 bg-white p-3">
                    ...
                    {selectedDateEvents.map((event) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="inline-flex h-9 w-9 items-center justify-center rounded-xl text-slate-500 transition hover:bg-white hover:text-rose-500"
                  ...
                  <div className="min-h-[74px] rounded-[16px] border border-dashed border-slate-200 bg-white px-3 py-3 text-[13px] leading-5 text-slate-500">
                ...
                <div className="relative flex items-center justify-end gap-2 pt-1">
                  <Button
                    type="button"
                    variant="outline"
                    className="h-9 rounded-xl border-slate-200 bg-white px-3.5 text-[13px] text-slate-700 hover:bg-white"
                  ...
                  {showCalendarTour && currentCalendarTourStep.id === 'create' ? (
                    <TourCard
                      ...
                      onPrevious={handleCalendarTourPrevious}
                      onClose={closeCalendarTour}
                      className="pointer-events-auto absolute right-0 bottom-full z-[80] mb-3 max-sm:fixed max-sm:inset-x-4 max-sm:top-auto max-sm:bottom-[5.75rem] max-sm:translate-x-0 max-sm:translate-y-0"
                    />
                  ) : null}
                </div>
              </div>
      ...
      <Dialog open={showPremiumUpsell} onOpenChange={setShowPremiumUpsell}>
        <DialogContent className="w-[min(calc(100vw-1.5rem),420px)] rounded-[24px] border border-slate-200 bg-white p-0 text-slate-900 shadow-[0_24px_70px_rgba(15,23,42,0.14)]">
          <div className="px-4 py-4 sm:px-5 sm:py-5">
            <PremiumUpsell
              title="Armá el plan para tu próximo parcial"
              description="Ya usaste los 3 exámenes del plan gratuito. Con Premium organizá toda la cursada y recibí recordatorios antes de cada fecha."
