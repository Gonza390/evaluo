...
function activateTab(label: string) {
  const tab = findStudyTab(label);
  if (!tab) return false;
  if (!isTabActive(label)) tab.click();
  return true;
}
...
function findPrimaryContentTarget(tab: string) {
  ...
  const first = visibleChildren(panel)[0];
  if (!first) return panel;

  if (tab === 'Tarjetas') {
    const generateButton = visibleChildren(first)
      .flatMap((child) => [child, ...Array.from(child.querySelectorAll<HTMLElement>('button'))])
      .find((element) => isVisible(element) && normalizedText(element).includes('Generar flashcards'));

    if (generateButton?.parentElement && isVisible(generateButton.parentElement)) {
      return generateButton.parentElement;
    }

    const second = visibleChildren(first)[0];
    return second ?? first;
  }
...
export function DemoMaterialGuidedTourV5({
  enabled,
  force = false,
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      } else if (event.key === 'ArrowRight' && stepReady && stepIndex < TOUR_STEPS.length - 1) {
        setStepIndex((current) => current + 1);
      } else if (event.key === 'ArrowLeft' && stepReady && stepIndex > 0) {
        setStepIndex((current) => current - 1);
      }
    };
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const tabRect = rectFromElement(tab, 'tab', safe, vw, vh, 2, 14);
      let contentRect = rectFromElement(content, 'content', safe, vw, vh, 3, 20);

      if (vw >= 1024) {
        const cardWidth = 300;
        const cardGap = 26;
        const contentLimit = vw - cardWidth - cardGap - safe;

        if (currentStep.tab !== 'Examen') {
          const right = Math.min(contentRect.right, Math.max(contentRect.left + 420, contentLimit));
          contentRect = {
            ...contentRect,
            right,
        ...
        const maxHeight = currentStep.tab === 'Tarjetas' ? 430 : currentStep.tab === 'Examen' ? 620 : 500;
        const bottom = Math.min(contentRect.bottom, contentRect.top + Math.min(maxHeight, vh - contentRect.top - safe));
        contentRect = {
          ...contentRect,
          bottom,
        ...
        const bottom = Math.min(contentRect.bottom, contentRect.top + Math.min(430, vh - contentRect.top - safe));
        contentRect = { ...contentRect, bottom, height: Math.max(1, bottom - contentRect.top) };
      }
  ...
  const cardStyle = useMemo(() => {
    ...
    if (currentStep.target === 'content' && viewport.width >= 1024) {
      return {
        ...
        top: clamp(primarySpotlight.top + 8, safe, viewport.height - height - safe),
      };
    }
  ...
  }, [cardHeight, currentStep.target, primarySpotlight, viewport.height, viewport.width]);
  ...
  const maskId = 'evaluo-demo-tour-mask-v5';
