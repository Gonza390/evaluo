...
export function ProfileCompletionPrivateCatalog({
  ...
  const universityChoices = useMemo(
    () =>
      universidadSearch.trim().length >= 2
        ? universityMatches.map((match) => match.university)
  ...
  const careerChoices = useMemo(
    () =>
      carreraSearch.trim().length >= 3
        ? careerMatches.map((match) => match.career)
        : carreras.slice(0, 8),
    ...
    void (async () => {
      try {
        ...
        if (effectiveUniversidadId) {
          ...
          const name = uni?.nombre ?? persisted?.universidadNombre ?? '';
  ...
  const resetAfterUniversity = useCallback(() => {
    setCarreraId('');
    setCarreraNombre('');
    setCarreraSearch('');
  ...
  async function addMissingSubject() {
    if (busy || !carreraId) return;
    setBusy(true);
    try {
      const result = await createPrivatePendingSubjectAction({ carreraId, materiaNombre: missingSubjectName });
      if (!result.success) {
      ...
      >
        <div className="px-5 pt-5 sm:px-8 sm:pt-6">
          <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400">
            ...
            {hasPending ? <span className="inline-flex items-center gap-1 text-indigo-600"><LockKeyhole className="h-3 w-3" /> Privado</span> : null}
          </div>
                ...
                <div className="mt-5">
                  ...
                  {universidadSearch.trim().length >= 2 ? (
                      ...
                      <div className="mt-3 rounded-2xl border border-indigo-100 bg-indigo-50/40 p-3">
                        ...
                        <div className="mt-2 space-y-2">
                          {universityChoices.map((uni) => (
                            <button key={uni.id} type="button" onClick={() => selectUniversity(uni)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3 text-left shadow-sm transition hover:border-indigo-300">
                    ...
                    <div className="mt-2 max-h-40 overflow-y-auto border-y border-slate-200">
                      {universityChoices.map((uni) => (
                        <button key={uni.id} type="button" onClick={() => selectUniversity(uni)} className="flex w-full items-center justify-between border-b border-slate-100 px-2 py-2.5 text-left text-sm font-medium text-slate-700 last:border-0 hover:text-indigo-700">{uni.nombre}<ArrowRight className="h-4 w-4 text-slate-300" /></button>
                      ))}
                    </div>
                  )}
                </div>
                <button type="button" disabled={universidadSearch.trim().length < 3} onClick={() => { setMissingUniversityName(universidadSearch); setMissingUniversityCity(''); setShowMissingUniversity(true); }} className="mt-3 flex h-10 w-full items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 transition hover:border-indigo-200 hover:text-indigo-700 disabled:cursor-not-allowed disabled:opacity-40">Mi universidad no aparece</button>
              </>
            ) : (
        ...
        {step === 2 ? (
          <div className="px-5 py-5 sm:px-8 sm:py-6">
            <DialogHeader className="text-left"><div className="flex items-start gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-indigo-600 text-white"><GraduationCap className="h-5 w-5" /></span><div><DialogTitle className="text-2xl font-bold tracking-[-0.04em] text-slate-950">¿Qué carrera estudiás?</DialogTitle><DialogDescription className="mt-1 text-sm leading-6 text-slate-500">{universidadNombre}</DialogDescription></div></div></DialogHeader>
            {!showMissingCareer ? (
              <>
                <div className="mt-5"><Label htmlFor="onboarding-career">Carrera</Label><Input id="onboarding-career" className="mt-2 h-11" value={carreraSearch} onChange={(e) => setCarreraSearch(e.target.value)} placeholder="Ej. Martillero" />
                  {loadingCarreras ? <div className="flex justify-center py-6"><Spinner size="sm" /></div> : carreraSearch.trim().length >= 3 ? (
                    careerChoices.length ? <div className="mt-3 rounded-2xl border border-indigo-100 bg-indigo-50/40 p-3"><p className="text-xs font-bold uppercase tracking-[0.12em] text-indigo-600">¿Buscabas alguna de estas?</p><div className="mt-2 space-y-2">{careerChoices.map((career) => <button key={career.id} type="button" onClick={() => selectCareer(career)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3 text-left shadow-sm transition hover:border-indigo-300"><span className="min-w-0 text-sm font-bold text-slate-950">{career.nombre}</span><span className="shrink-0 text-xs font-bold text-indigo-600">Usar esta carrera</span></button>)}</div></div> : <p className="mt-3 rounded-xl bg-slate-50 px-4 py-2.5 text-sm text-slate-500">No encontramos una carrera suficientemente parecida.</p>
                  ) : <div className="mt-2 max-h-40 overflow-y-auto border-y border-slate-200">{careerChoices.map((career) => <button key={career.id} type="button" onClick={() => selectCareer(career)} className="flex w-full items-center justify-between border-b border-slate-100 px-2 py-2.5 text-left text-sm font-medium text-slate-700 last:border-0 hover:text-indigo-700">{career.nombre}<ArrowRight className="h-4 w-4 text-slate-300" /></button>)}</div>}
                </div>
                <button type="button" disabled={carreraSearch.trim().length < 3} onClick={() => { setMissingCareerName(carreraSearch); setShowMissingCareer(true); }} className="mt-3 flex h-10 w-full items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 transition hover:border-indigo-200 hover:text-indigo-700 disabled:cursor-not-allowed disabled:opacity-40">No es ninguna de estas · Solicitar nueva carrera</button>
              </>
            ) : (
              <div className="mt-5 space-y-4 border-y border-slate-200 py-5">
                <div><Label htmlFor="missing-career">Nombre de tu carrera</Label><Input id="missing-career" className="mt-2 h-11" value={missingCareerName} onChange={(e) => setMissingCareerName(e.target.value)} placeholder="Ej. Licenciatura en Psicología" /></div>
                <div><Label htmlFor="missing-faculty">Facultad o unidad académica <span className="font-normal text-slate-400">(opcional)</span></Label><Input id="missing-faculty" className="mt-2 h-11" value={missingFacultyName} onChange={(e) => setMissingFacultyName(e.target.value)} placeholder="Ej. Facultad de Psicología" /></div>
                <div className="flex items-start gap-2 text-xs leading-5 text-slate-500"><LockKeyhole className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" /><p>La carrera queda asignada sólo a tu perfil. No aparecerá para otros estudiantes hasta que Evaluo la revise y apruebe.</p></div>
                <div className="flex gap-2"><Button variant="outline" type="button" onClick={() => setShowMissingCareer(false)}>Cancelar</Button><Button type="button" disabled={busy || missingCareerName.trim().length < 3} onClick={() => void addMissingCareer()} className="bg-indigo-600 hover:bg-indigo-700">{busy ? 'Guardando...' : 'Enviar solicitud y seguir'}</Button></div>
              </div>
            )}
          </div>
        ) : null}
        ...
        {step === 3 ? (
          <div className="px-5 py-5 sm:px-8 sm:py-6">
            <DialogHeader className="text-left"><div className="flex items-start gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-indigo-600 text-white"><Sparkles className="h-5 w-5" /></span><div><DialogTitle className="text-2xl font-bold tracking-[-0.04em] text-slate-950">¿Qué materias cursás?</DialogTitle><DialogDescription className="mt-1 text-sm leading-6 text-slate-500">Seleccioná al menos una. Si falta una materia, también podés agregarla para tu uso privado.</DialogDescription></div></div></DialogHeader>
            {pendingUniversity || pendingCareer ? <div className="mt-4 flex items-start gap-2 border-y border-indigo-100 bg-indigo-50/60 px-1 py-3 text-xs leading-5 text-indigo-800"><LockKeyhole className="mt-0.5 h-4 w-4 shrink-0" /><p>Tu universidad o carrera está pendiente de aprobación. Sólo vos podés verla y cualquier PDF asociado quedará privado mientras tanto.</p></div> : null}
            <div className="mt-4 max-h-48 overflow-y-auto border-y border-slate-200">{loadingMaterias ? <div className="flex justify-center py-8"><Spinner size="sm" /></div> : materias.length ? materias.map((materia) => { const selected = selectedMateriaIds.includes(materia.id); const pending = pendingMateriaIds.includes(materia.id); return <button key={materia.id} type="button" onClick={() => toggleSubject(materia.id)} className="flex w-full items-center gap-3 border-b border-slate-100 px-2 py-2.5 text-left last:border-0"><span className={`flex h-4 w-4 items-center justify-center rounded border ${selected ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-300'}`}>{selected ? <Check className="h-3 w-3" /> : null}</span><span className="min-w-0 flex-1 text-sm font-medium text-slate-700">{materia.nombre}</span>{pending ? <span className="text-[10px] font-bold uppercase tracking-[0.08em] text-indigo-600">Privada</span> : null}</button>; }) : <p className="py-6 text-center text-sm text-slate-500">Todavía no tenemos materias cargadas para esta carrera.</p>}</div>
            {!showMissingSubject ? <button type="button" onClick={() => setShowMissingSubject(true)} className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-indigo-700"><Plus className="h-4 w-4" /> Agregar una materia que falta</button> : <div className="mt-3 border-y border-slate-200 py-3"><Label htmlFor="missing-subject">Nombre de la materia</Label><div className="mt-2 flex gap-2"><Input id="missing-subject" className="h-11" value={missingSubjectName} onChange={(e) => setMissingSubjectName(e.target.value)} placeholder="Ej. Biología" /><Button type="button" disabled={busy || missingSubjectName.trim().length < 2} onClick={() => void addMissingSubject()} className="bg-indigo-600 hover:bg-indigo-700">{busy ? <Spinner size="sm" /> : 'Agregar'}</Button></div><p className="mt-2 text-xs leading-5 text-slate-500">Quedará visible sólo para vos hasta que sea aprobada.</p></div>}
          </div>
        ) : null}

        {step === 4 ? hasMissingCatalog ? (
          <div className="px-5 py-6 sm:px-8 sm:py-7">
            <div className="text-center"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600"><Check className="h-6 w-6" /></span><DialogTitle className="mt-4 text-2xl font-bold tracking-[-0.04em] text-slate-950">Empezá con tu primer material</DialogTitle><DialogDescription className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">Tu solicitud quedó registrada. Podés subir un PDF propio o ver un ejemplo para conocer cómo lo transforma Evaluo.</DialogDescription></div>
            <div className="mt-5 grid gap-3 sm:grid-cols-[1.15fr_0.85fr]">
              <a href="/dashboard/materiales/subir?source=onboarding_missing_catalog" className="group relative rounded-2xl border-2 border-indigo-200 bg-indigo-50/40 p-5 text-left shadow-[0_10px_28px_rgba(79,70,229,0.08)] transition hover:border-indigo-300 hover:shadow-[0_14px_32px_rgba(79,70,229,0.12)]"><span className="absolute right-3 top-3 rounded-full border border-indigo-100 bg-white px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.1em] text-indigo-600">Recomendado</span><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-100 text-indigo-600"><FileUp className="h-5 w-5" /></span><h2 className="mt-3 text-lg font-bold text-slate-950">Subir mi PDF</h2><p className="mt-1 text-sm leading-5 text-slate-500">Usá tu propio material y empezá a prepararlo para estudiar.</p><span className="mt-3 inline-flex items-center gap-1.5 text-sm font-bold text-indigo-600">Elegir PDF <ArrowRight className="h-4 w-4" /></span></a>
              <a href="/demo/material-estudio?source=onboarding_missing_catalog" className="group rounded-2xl border border-slate-200 bg-white p-4 text-left transition hover:border-indigo-200 hover:bg-indigo-50/20"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-slate-600 group-hover:bg-indigo-50 group-hover:text-indigo-600"><FileText className="h-5 w-5" /></span><h2 className="mt-3 text-base font-bold text-slate-950">Ver un PDF de ejemplo</h2><p className="mt-1 text-sm leading-5 text-slate-500">Mirá cómo queda un material dentro de Evaluo.</p></a>
            </div>
          ...
          <div className="px-5 py-9 text-center sm:px-8 sm:py-10"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500 text-white"><Check className="h-7 w-7" /></span><DialogTitle className="mt-5 text-2xl font-bold tracking-[-0.04em] text-slate-950">Tu espacio está listo</DialogTitle><DialogDescription className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">{hasPending ? 'Podés empezar a estudiar ahora. Las materias nuevas quedan sólo en tu cuenta hasta que sean aprobadas.' : 'Ya configuramos tu universidad, carrera y materias activas.'}</DialogDescription><Button type="button" onClick={onComplete} className="mt-6 h-11 w-full bg-indigo-600 font-semibold hover:bg-indigo-700">Empezar a estudiar <ArrowRight className="ml-2 h-4 w-4" /></Button></div>
        ) : null}

        {step > 1 && step < 4 ? <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3 sm:px-8"><Button type="button" variant="outline" onClick={() => setStep((current) => Math.max(1, current - 1))}><ArrowLeft className="mr-2 h-4 w-4" /> Atrás</Button>{step === 3 ? <Button type="button" disabled={busy || selectedMateriaIds.length === 0} onClick={() => void saveProfile()} className="bg-indigo-600 hover:bg-indigo-700">{busy ? 'Guardando...' : 'Continuar'} <ArrowRight className="ml-2 h-4 w-4" /></Button> : null}</div> : null}
      </DialogContent>
    </Dialog>
  );
}
