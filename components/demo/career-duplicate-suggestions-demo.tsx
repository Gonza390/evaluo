...
export function CareerDuplicateSuggestionsDemo({ universityName, careers }: Props) {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#eef2ff_0,white_42%)] px-4 py-8 sm:py-12">
      <div className="mx-auto w-full max-w-xl">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-indigo-600">Preview · onboarding</p>
            <h1 className="mt-1 text-xl font-bold tracking-[-0.04em] text-slate-950">Evitar carreras duplicadas</h1>
          </div>
        ...
        <section className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-[0_28px_80px_rgba(15,23,42,0.12)]">
          <div className="border-b border-slate-100 px-5 pt-5 sm:px-8 sm:pt-6">
            ...
            <div className="mt-2 flex gap-1.5 pb-5">
          ...
          <div className="px-5 py-6 sm:px-8 sm:py-7">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="text-2xl font-bold tracking-[-0.04em] text-slate-950">¿Qué carrera estudiás?</h2>
                <p className="mt-1 text-sm leading-6 text-slate-500">{universityName}. Si no aparece, comprobamos primero si ya existe con otro nombre.</p>
              </div>
            </div>
            ...
            {selected ? (
              <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-5">
                ...
                <button type="button" onClick={() => setSelected(null)} className="mt-4 text-sm font-semibold text-indigo-700">Probar otra búsqueda</button>
              </div>
            ) : guardState === 'confirm' && strongMatch ? (
              <div className="mt-6 rounded-2xl border border-indigo-200 bg-gradient-to-br from-indigo-50 to-white p-5 shadow-[0_14px_34px_rgba(79,70,229,0.08)]">
                ...
                <h3 className="mt-3 text-lg font-bold tracking-[-0.03em] text-slate-950">Encontramos una carrera muy parecida que ya está disponible.</h3>
                <div className="mt-4 rounded-xl border border-indigo-100 bg-white p-4">
                  ...
                  <p className="mt-1 text-sm text-slate-500">{universityName}</p>
                </div>
                <button type="button" onClick={() => useCareer(strongMatch.career)} className="mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 text-sm font-bold text-white transition hover:bg-indigo-700">Usar esta carrera <ArrowRight className="h-4 w-4" /></button>
                <button type="button" onClick={() => setGuardState('created')} className="mt-3 w-full text-center text-sm font-semibold text-slate-500 hover:text-slate-800">No, es otra carrera</button>
              </div>
            ) : guardState === 'created' ? (
              <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-center">
                ...
                <p className="mt-1 text-sm leading-6 text-slate-500">En el onboarding real, recién acá se crea la carrera pendiente. Este preview no modifica datos.</p>
                <button type="button" onClick={() => setGuardState('idle')} className="mt-3 text-sm font-semibold text-indigo-700">Volver</button>
              </div>
            ) : (
              <>
                <label htmlFor="career-demo" className="mt-6 block text-sm font-semibold text-slate-800">Nombre de tu carrera</label>
                <div className="relative mt-2">
                  ...
                  <input
                    ...
                    className="h-12 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-sm text-slate-900 outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50"
                  />
                </div>

                {matches.length ? (
                  <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50/40 p-4">
                    <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-indigo-600"><Sparkles className="h-4 w-4" /> ¿Buscabas alguna de estas?</div>
                    <p className="mt-1 text-xs leading-5 text-slate-500">Ya están disponibles en Evaluo.</p>
                    <div className="mt-3 space-y-2">
                      {matches.map(({ career }) => (
                        <button
                          ...
                          className="group flex w-full items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3.5 text-left shadow-sm transition hover:border-indigo-300 hover:shadow-md"
                        >
                          <div className="min-w-0">
                            <p className="font-bold text-slate-950">{career.nombre}</p>
                  ...
                  <p className="mt-4 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-500">No encontramos una carrera suficientemente parecida.</p>
                ) : null}
                ...
                <button
                  ...
                  className="mt-5 flex h-11 w-full items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 transition hover:border-indigo-200 hover:text-indigo-700 disabled:cursor-not-allowed disabled:opacity-40"
                >
