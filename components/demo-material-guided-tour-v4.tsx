...
function resetStudyPosition() {
  const studyRegion = findStudyRegion();
  if (studyRegion) studyRegion.scrollTop = 0;

  const activePanel = findContentTarget();
...
export function DemoMaterialGuidedTourV4({
  enabled,
  force = false,
  ...
  const [cardSize, setCardSize] = useState({ width: 300, height: 290 });
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const content = findContentTarget();
      if (!tab || !content) {
        timer = window.setTimeout(measure, 100);
        return;
      }

      const tabRect = rectFromElement(tab, 'tab', safe, vw, vh, 5, 15);
      let contentRect = rectFromElement(content, 'content', safe, vw, vh, 4, 20);

      if (vw >= 1024 && currentStep.contentMode === 'preview') {
        const reservedCard = 300 + 28 + safe;
        const maxRight = Math.max(contentRect.left + 440, vw - reservedCard);
        const right = Math.min(contentRect.right, maxRight);
        const maxHeight = Math.min(500, Math.max(280, vh - contentRect.top - safe));
        const bottom = Math.min(contentRect.bottom, contentRect.top + maxHeight);
        contentRect = {
          ...contentRect,
          right,
          bottom,
          width: Math.max(1, right - contentRect.left),
          height: Math.max(1, bottom - contentRect.top),
        };
      } else if (vw >= 1024 && currentStep.contentMode === 'full') {
        const maxHeight = Math.min(620, Math.max(360, vh - contentRect.top - safe));
        const bottom = Math.min(contentRect.bottom, contentRect.top + maxHeight);
        contentRect = {
          ...contentRect,
          bottom,
        ...
        const maxHeight = Math.min(430, Math.max(260, vh - contentRect.top - safe));
        const bottom = Math.min(contentRect.bottom, contentRect.top + maxHeight);
        contentRect = {
          ...contentRect,
          bottom,
      ...
      setSpotlights([tabRect, contentRect]);
    };

    timer = window.setTimeout(measure, currentStep.target === 'pdf' ? 180 : 120);
  ...
  }, [currentStep.contentMode, currentStep.tab, currentStep.target, open, stepIndex, stepReady]);
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setCardSize({ width: rect.width || 300, height: rect.height || 290 });
    };
  ...
  const primarySpotlight = spotlights.find((spotlight) => spotlight.kind === 'content') ?? spotlights[0];

  const cardStyle = useMemo(() => {
    if (!primarySpotlight || viewport.width < 768) return undefined;
    ...
    if (currentStep.target === 'content' && viewport.width >= 1024) {
      return {
        ...
        top: clamp(primarySpotlight.top + 10, safe, viewport.height - height - safe),
      };
    }

    const gap = 22;
    const roomLeft = primarySpotlight.left - safe;
    const roomRight = viewport.width - primarySpotlight.right - safe;
    const left =
      roomLeft >= width + gap
        ? primarySpotlight.left - width - gap
        : roomRight >= width + gap
          ? primarySpotlight.right + gap
          : clamp((viewport.width - width) / 2, safe, viewport.width - width - safe);

    return {
      width,
      left,
      top: clamp(primarySpotlight.top + 6, safe, viewport.height - height - safe),
    };
  }, [cardSize.height, currentStep.target, primarySpotlight, viewport.height, viewport.width]);
  ...
  const maskId = 'evaluo-demo-tour-mask-v4';

  return (
    <div className="fixed inset-0 z-[120] overflow-hidden" aria-live="polite">
      {stepReady && spotlights.length > 0 ? (
        <>
          ...
          {spotlights.map((spotlight, index) => (
            <div
              ...
              className="pointer-events-none absolute border-2 border-indigo-300 shadow-[0_0_0_3px_rgba(255,255,255,0.48),0_14px_36px_rgba(15,23,42,0.16)]"
              style={{
                top: spotlight.top,
                left: spotlight.left,
                width: spotlight.width,
      ...
      {stepReady ? (
        <section
          ...
          className={`fixed max-h-[calc(100svh-24px)] overflow-y-auto rounded-[22px] border border-slate-200 bg-white shadow-[0_24px_68px_rgba(15,23,42,0.25)] ${
            primarySpotlight && viewport.width >= 768
              ? ''
              : finalStep
                ? 'left-1/2 top-1/2 w-[calc(100%-24px)] max-w-[420px] -translate-x-1/2 -translate-y-1/2'
                : 'bottom-3 left-3 right-3'
          }`}
          style={primarySpotlight && viewport.width >= 768 ? cardStyle : undefined}
        >
